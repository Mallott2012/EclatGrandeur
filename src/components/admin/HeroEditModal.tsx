...
import { uploadHeroMediaAction } from '@/app/admin/(console)/hero/actions';
...
export function HeroEditModal({
  ...
  function handleMediaUploaded(url: string) {
    setStoragePath(url);
    if (url.toLowerCase().endsWith('.mp4')) setMediaType('video');
    else setMediaType('image');
  }

  function handleSave() {
    if (!storagePath.trim()) { setError('Please upload or enter a media file.'); return; }
    setError('');
        ...
        <div className="px-8 py-8 space-y-7">
          ...
          <div>
            ...
            <div className="mt-3 flex items-center gap-4">
              <MediaDropZone
                onUpload={uploadHeroMediaAction}
                onUploaded={handleMediaUploaded}
                disabled={pending}
              />
              <p className="font-sans" style={{ fontSize: 11, color: '#bbb', letterSpacing: '0.02em', lineHeight: 1.6 }}>
                Drop, paste (Ctrl+V), or click to upload.<br />
                JPEG, PNG, WebP, AVIF or MP4.
              </p>
            </div>
            {storagePath && (
              <div className="mt-3 flex items-center gap-2">
                <p className="font-sans flex-1 truncate" style={{ fontSize: 10, color: '#aaa', letterSpacing: '0.02em' }}>
                  {storagePath}
                </p>
                <button
                  type="button"
                  onClick={() => setStoragePath('')}
                  className="font-sans uppercase transition-opacity hover:opacity-60 flex-shrink-0"
                  style={{ fontSize: 9, letterSpacing: '0.15em', color: '#b85a5a' }}
                >
                  Remove
                </button>
              </div>
