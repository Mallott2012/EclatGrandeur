...
import {
  EARRING_STONE_ACTION_INITIAL,
  type EarringStoneActionResult,
...
const DEFAULT: DefaultValues = {
  sku: '', earring_setting_id: '', stone_type: 'diamond', shape: '', carat: '',
...
interface Props {
  action: (state: EarringStoneActionResult, formData: FormData) => Promise<EarringStoneActionResult>
  settings: SettingOption[]
  defaultValues?: Partial<DefaultValues>
}

export function EarringStoneForm({ action, settings, defaultValues }: Props) {
  const [state, formAction] = useActionState(action, EARRING_STONE_ACTION_INITIAL)
  ...
  return (
    <form action={formAction} className="space-y-6">
      ...
      <Section title="Identity">
        ...
        <Field label="Linked earring setting" error={fe['earring_setting_id']}>
          <select name="earring_setting_id" defaultValue={v.earring_setting_id} className={inputCls(false)}>
...
function SubmitButton() {
  ...
  return (
    <button type="submit" disabled={pending}
      ...
      {pending ? 'Saving…' : 'Save earring stone'}
    </button>
  )
}
