...
import { NECKLACE_STYLES, NECKLACE_STYLE_LABELS } from '@/lib/necklace-settings/types'
import {
  NECKLACE_SETTING_ACTION_INITIAL,
  type NecklaceSettingActionResult,
...
const DEFAULT: DefaultValues = {
  ...
  metals: ['platinum'], chain_lengths_cm: [], base_price_gbp: '', status: 'available',
  is_published: false, sort_order: 0,
}

interface Props {
  action: (state: NecklaceSettingActionResult, formData: FormData) => Promise<NecklaceSettingActionResult>
  defaultValues?: Partial<DefaultValues>
}

export function NecklaceSettingForm({ action, defaultValues }: Props) {
  const [state, formAction] = useActionState(action, NECKLACE_SETTING_ACTION_INITIAL)
    ...
    <form action={formAction} className="space-y-6">
      ...
      <Field label="Name" error={fieldErrors['name']} required>
        <input type="text" name="name" id="name" defaultValue={values.name} onBlur={handleNameBlur}
          className={inputCls(!!fieldErrors['name'])} placeholder="e.g. The Constance Pendant" />
      </Field>

      <Field label="Slug (URL)" error={fieldErrors['slug']} required>
        <input type="text" name="slug" id="slug" defaultValue={values.slug}
          className={inputCls(!!fieldErrors['slug'])} placeholder="e.g. constance-pendant" />
      ...
      <Field label="Style" error={fieldErrors['style']}>
        <select name="style" defaultValue={values.style} className={inputCls(false)}>
          ...
          {NECKLACE_STYLES.map((s) => (
            <option key={s} value={s}>{NECKLACE_STYLE_LABELS[s]}</option>
          ))}
        </select>
      </Field>
      ...
      <Field label="Chain lengths (cm)" error={fieldErrors['chain_lengths_cm']}>
        <input type="text" name="chain_lengths_cm" defaultValue={values.chain_lengths_cm.join(', ')}
          className={inputCls(false)} placeholder="e.g. 40, 45, 50" />
        <p className="mt-1 text-xs text-neutral-600">Comma-separated lengths in centimetres.</p>
      </Field>
      ...
      <Field label="Base price (GBP)" error={fieldErrors['base_price_gbp']}>
        <div className="flex items-center gap-1">
          ...
          <input type="number" name="base_price_gbp" defaultValue={values.base_price_gbp} step="0.01" min="0"
            className={inputCls(false)} placeholder="e.g. 1800" />
        </div>
      </Field>

      <Field label="Description" error={fieldErrors['description']}>
        <textarea name="description" defaultValue={values.description} rows={4}
          className={inputCls(false) + ' resize-y'} placeholder="Describe the craftsmanship and design of this necklace..." />
      </Field>
...
function SubmitButton() {
  ...
  return (
    <button type="submit" disabled={pending}
      ...
      {pending ? 'Saving…' : 'Save necklace setting'}
    </button>
  )
}
