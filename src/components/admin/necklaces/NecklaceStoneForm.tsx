...
import {
  NECKLACE_STONE_ACTION_INITIAL,
  type NecklaceStoneActionResult,
} from '@/app/admin/(console)/necklace-stones/types'
...
const DEFAULT: DefaultValues = {
  sku: '', necklace_setting_id: '', stone_type: 'diamond', shape: '', carat: '',
...
interface Props {
  action: (state: NecklaceStoneActionResult, formData: FormData) => Promise<NecklaceStoneActionResult>
  settings: SettingOption[]
  defaultValues?: Partial<DefaultValues>
}

export function NecklaceStoneForm({ action, settings, defaultValues }: Props) {
  const [state, formAction] = useActionState(action, NECKLACE_STONE_ACTION_INITIAL)
  ...
  return (
    <form action={formAction} className="space-y-6">
      ...
      <Section title="Identity">
        ...
        <Field label="Linked necklace setting" error={fe['necklace_setting_id']}>
          <select name="necklace_setting_id" defaultValue={v.necklace_setting_id} className={inputCls(false)}>
...
function SubmitButton() {
  ...
  return (
    <button type="submit" disabled={pending}
      ...
      {pending ? 'Saving…' : 'Save necklace stone'}
    </button>
  )
}
