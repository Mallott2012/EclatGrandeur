...
/**
 * Premium "available following a private consultation" state for an earring that
 * is routed through the new earring architecture but currently has no selectable
 * matched-pair inventory (State 2).
 *
 * Pure server component. Never mounts DiamondSelector, never calls /api/diamonds,
 * never shows a Choose-Diamonds button or an empty/broken configuration section.
 * Exposes no internal flags, hold tokens, or operational data.
 */
  ...
  return (
    <div className="min-h-screen bg-white pb-10 lg:pb-20" style={{ color: G }}>
      ...
      >
        ...
        <Link href={categoryPath} className="font-sans" style={{ fontSize: 11, color: '#bbb', letterSpacing: '0.1em', textTransform: 'uppercase' }}>
          {categoryLabel}
        </Link>
        <span style={{ color: '#ddd' }}>·</span>
      ...
      <div className="flex flex-col lg:flex-row">
        ...
        <div className="lg:w-[42%] px-8 lg:px-12 pt-12 pb-20 flex flex-col">
          ...
          {productSubtitle && (
            <p className="font-sans mt-2" style={{ fontSize: 13, color: '#999', fontWeight: 300, letterSpacing: '0.03em' }}>
              {productSubtitle}
            </p>
          )}
          ...
          {/* Availability state — premium, intentional, no broken chooser */}
          <div className="mt-8">
            <p className="font-sans uppercase" style={{ fontSize: 10, letterSpacing: '0.28em', color: '#b08d57' }}>
            ...
            <p className="font-sans mt-4" style={{ fontSize: 13, color: '#666', lineHeight: 1.85, fontWeight: 300 }}>
              Each pair of these earrings is composed from a matched set of diamonds selected to your
              preference. Our diamond specialists will guide you through the curated pairs currently
              available and reserve the perfect match on your behalf.
            </p>
          </div>

          <Link
            href="/contact"
            className="font-sans uppercase mt-8 py-4 text-center"
          ...
          <Link
            href={categoryPath}
            className="font-sans uppercase mt-3 py-3 text-center transition-opacity hover:opacity-70"
