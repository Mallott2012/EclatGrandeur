...
export async function GET(request: Request) {
  try {
    ...
    if (ringSettingId && metal) {
      ...
      assignedIds = (data ?? []).map((r: { diamond_id: string }) => r.diamond_id)
      // No assignments → return empty list immediately
      if (assignedIds.length === 0) return NextResponse.json({ diamonds: [] })
    }
