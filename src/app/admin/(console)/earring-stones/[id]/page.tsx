...
import { getEarringStone } from '@/lib/earring-stones/service'
...
import { DeleteEarringStoneForm } from '@/components/admin/earrings/DeleteEarringStoneForm'
import { deleteEarringStoneAction } from './actions'

interface Props { params: Promise<{ id: string }> }

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { id } = await params
  const stone = await getEarringStone(id)
  return {
    title: stone ? `${stone.sku} — Admin` : 'Earring Stone — Admin',
    robots: { index: false, follow: false },
  }
}

export default async function EarringStoneDetailPage({ params }: Props) {
  await requireStaffRole(['super_admin'])
  const { id } = await params
  const stone = await getEarringStone(id)
  if (!stone) notFound()

  const deleteWithId = deleteEarringStoneAction.bind(null, id)
    ...
    <div className="max-w-3xl">
      ...
      <div className="mb-8 flex items-start justify-between">
        ...
        <div className="flex items-center gap-2">
          <Link href={`/admin/earring-stones/${id}/edit`}
          ...
          <DeleteEarringStoneForm deleteAction={deleteWithId} />
        </div>
      </div>
