  ...
  return (
    <div className="max-w-5xl">
      ...
      <div className="mb-8 flex items-start justify-between">
        <div>
          <h1 className="font-display text-3xl font-light tracking-widest text-white">RING SETTINGS</h1>
      ...
      {items.length === 0 ? (
        <div className="rounded border border-neutral-800 bg-neutral-900/50 px-6 py-16 text-center">
          <p className="text-sm text-neutral-500">No ring settings yet.</p>
          <Link
            href="/admin/ring-settings/new"
            className="mt-4 inline-block text-xs tracking-widest text-amber-600 hover:text-amber-400"
          >
            ...
            <tbody className="divide-y divide-neutral-800/60">
              {items.map((s) => (
                <tr key={s.id} className="transition-colors hover:bg-neutral-900/40">
                  ...
                  <td className="px-4 py-3 text-neutral-400">{s.collection ?? '—'}</td>
                  <td className="px-4 py-3 text-neutral-400">
                    <div className="flex flex-wrap gap-1">
                      {s.metals.map((m) => (
                        <span key={m} className="rounded bg-neutral-800 px-1.5 py-0.5 text-xs text-neutral-300">
                          {METAL_LABELS[m]}
                        </span>
                      ))}
                    </div>
