...
import { DeleteNecklaceSettingForm } from '@/components/admin/necklaces/DeleteNecklaceSettingForm'
import { deleteNecklaceSettingAction } from './actions'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: setting ? `${setting.name} — Admin` : 'Necklace Setting — Admin',
    robots: { index: false, follow: false },
  }
}

export default async function NecklaceSettingDetailPage({ params }: Props) {
  await requireStaffRole(['super_admin'])
  const { id } = await params
  const setting = await getNecklaceSetting(id)
  if (!setting) notFound()

  const deleteWithId = deleteNecklaceSettingAction.bind(null, id)
    ...
    <div className="max-w-3xl">
      ...
      <div className="mb-8 flex items-start justify-between">
        ...
        <div className="flex items-center gap-2">
          <Link href={`/admin/necklace-settings/${id}/edit`}
          ...
          <DeleteNecklaceSettingForm deleteAction={deleteWithId} />
        </div>
      </div>
      ...
      <div className="space-y-6">
        <section className="rounded border border-neutral-800 bg-neutral-900/30 p-6">
          ...
          <dl className="space-y-0.5">
            ...
            <Row label="Chain lengths" value={setting.chain_lengths_cm.length ? setting.chain_lengths_cm.map((c) => `${c} cm`).join(', ') : '—'} />
