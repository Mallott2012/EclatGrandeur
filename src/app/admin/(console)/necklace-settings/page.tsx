...
import { listNecklaceSettings } from '@/lib/necklace-settings/service'
import { METAL_LABELS } from '@/lib/catalogue/enums'

export const metadata: Metadata = {
  title: 'Necklace Settings — Éclat Grandeur Admin',
  robots: { index: false, follow: false },
}

export default async function NecklaceSettingsListPage() {
  await requireStaffRole(['super_admin'])
  const items = await listNecklaceSettings()

  return (
    <div className="max-w-5xl">
      <div className="mb-8 flex items-start justify-between">
        <div>
          <h1 className="font-display text-3xl font-light tracking-widest text-white">NECKLACE SETTINGS</h1>
          <p className="mt-1 text-sm text-neutral-400">{items.length} setting{items.length !== 1 ? 's' : ''} total</p>
        </div>
        <Link href="/admin/necklace-settings/new"
      ...
      {items.length === 0 ? (
        <div className="rounded border border-neutral-800 bg-neutral-900/50 px-6 py-16 text-center">
          <p className="text-sm text-neutral-500">No necklace settings yet.</p>
          <Link href="/admin/necklace-settings/new" className="mt-4 inline-block text-xs tracking-widest text-amber-600 hover:text-amber-400">
            Create your first necklace setting →
          </Link>
        </div>
      ) : (
              ...
              {items.map((s) => (
                <tr key={s.id} className="transition-colors hover:bg-neutral-900/40">
                  ...
                  <td className="px-4 py-3 text-right">
                    <Link href={`/admin/necklace-settings/${s.id}`} className="text-xs text-amber-600 hover:text-amber-400">View →</Link>
                  </td>
                </tr>
              ))}
            </tbody>
