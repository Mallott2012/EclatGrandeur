...
import { getBraceletStone } from '@/lib/bracelet-stones/service'
...
import { DeleteBraceletStoneForm } from '@/components/admin/bracelets/DeleteBraceletStoneForm'
import { deleteBraceletStoneAction } from './actions'

interface Props { params: Promise<{ id: string }> }

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { id } = await params
  const stone = await getBraceletStone(id)
  return {
    title: stone ? `${stone.sku} — Admin` : 'Bracelet Stone — Admin',
    robots: { index: false, follow: false },
  }
}

export default async function BraceletStoneDetailPage({ params }: Props) {
  await requireStaffRole(['super_admin'])
  const { id } = await params
  const stone = await getBraceletStone(id)
  if (!stone) notFound()

  const deleteWithId = deleteBraceletStoneAction.bind(null, id)
    ...
    <div className="max-w-3xl">
      ...
      <div className="mb-8 flex items-start justify-between">
        ...
        <div className="flex items-center gap-2">
          <Link href={`/admin/bracelet-stones/${id}/edit`}
          ...
          <DeleteBraceletStoneForm deleteAction={deleteWithId} />
        </div>
      </div>
