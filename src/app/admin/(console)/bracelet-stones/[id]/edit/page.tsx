...
import { getBraceletStone, listBraceletSettingOptions } from '@/lib/bracelet-stones/service'
import { BraceletStoneForm } from '@/components/admin/bracelets/BraceletStoneForm'
import { updateBraceletStoneAction } from '../actions'

interface Props { params: Promise<{ id: string }> }

export const metadata: Metadata = {
  title: 'Edit Bracelet Stone — Éclat Grandeur Admin',
  robots: { index: false, follow: false },
}

export default async function EditBraceletStonePage({ params }: Props) {
  await requireStaffRole(['super_admin'])
  const { id } = await params
  const [stone, settings] = await Promise.all([getBraceletStone(id), listBraceletSettingOptions()])
  if (!stone) notFound()

  const boundAction = updateBraceletStoneAction.bind(null, id)

  return (
    <div className="max-w-2xl">
      <Link href={`/admin/bracelet-stones/${id}`}
        className="mb-4 inline-block text-xs tracking-widest text-neutral-500 transition-colors hover:text-neutral-300">
        ← {stone.sku}
      </Link>
      <h1 className="mb-8 font-display text-3xl font-light tracking-widest text-white">EDIT BRACELET STONE</h1>
      <BraceletStoneForm
        action={boundAction}
        settings={settings}
        defaultValues={{
          sku:                 stone.sku,
          bracelet_setting_id: stone.bracelet_setting_id ?? '',
